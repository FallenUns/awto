...
- **DECISION — multi-provider cloud, design-first / implement-later (remember this)**: the cloud Provider dropdown ([src/options/LLMTab.tsx](src/options/LLMTab.tsx) `PROVIDERS`) renders Anthropic / OpenAI / Google Gemini / OpenRouter / Custom, but **only Anthropic is wired to the backend today**. `LLMSettings` is still Anthropic-only (`anthropicApiKey`/`anthropicModel`); non-Anthropic key/model selections live in local component state (not persisted) and show a "preview — not wired yet" note. Backend multi-provider support (per-provider key/model persistence + routing in `hybrid.ts`/`cloud.ts`) is the follow-up. Do **not** assume non-Anthropic works until that lands.
