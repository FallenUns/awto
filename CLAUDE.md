...
### 18. Local LLM malformed-output retry
- **2026-06-30**: Weak local models (llama3.2:3b) intermittently emit empty/partial mapping objects under structured output (`{"mappings":[{}]}`), which failed Zod validation. `callLocal` threw immediately; local-only users (no cloud key) saw the raw Zod error dump (`mappings[0].fieldId Required …`).
- **Fix** ([src/background/llm/local.ts](src/background/llm/local.ts)): `callLocal` now retries **once** on unparseable/schema-invalid output — second attempt uses `temperature 0.5` (breaks deterministic bad output) + an appended repair instruction. Transport/HTTP/403/timeout/abort errors are extracted into `performChat` and thrown immediately (never retried, so hybrid's cloud fallback still triggers). If the retry also fails, a clear actionable message ("try a larger model / enable cloud fallback") replaces the raw Zod dump. Attempt 0 is byte-identical to the old request, so the happy path is unchanged.

### 19. Ollama model catalog + in-app download/delete
- **2026-06-30**: Spec [docs/superpowers/specs/2026-06-30-model-catalog-and-management-design.md](docs/superpowers/specs/2026-06-30-model-catalog-and-management-design.md) / plan [docs/superpowers/plans/2026-06-30-model-catalog-and-management.md](docs/superpowers/plans/2026-06-30-model-catalog-and-management.md). Built via subagent-driven TDD.
- **What**: Options "Local model" field replaced by a curated catalog ([src/options/model-catalog.ts](src/options/model-catalog.ts)) of general-purpose **instruct** models (reasoning models excluded — `<think>` breaks structured output), grouped Recommended (light + balanced, `qwen2.5:7b` = ★) vs Heavy. [src/options/ModelCatalog.tsx](src/options/ModelCatalog.tsx) merges the catalog with `/api/tags` (installed) + `navigator.deviceMemory` (soft RAM hint).
- **Download/delete from the page, not the SW**: [src/options/ollama-manage.ts](src/options/ollama-manage.ts) `pullModel` streams `/api/pull` NDJSON progress; `deleteModel` calls `/api/delete`. Run from the Options page because a multi-minute pull would be killed by MV3 service-worker idle termination. `OLLAMA_ORIGINS_HELP` extracted to [src/shared/ollama-errors.ts](src/shared/ollama-errors.ts), re-exported by `local.ts`.
- **Hardware**: `isHeavyForDevice(model, deviceMemoryGB)` (true only when known and `ramGB > device`). Disclaimers on two surfaces — settings (per-model amber flag + Heavy-group banner) and the popup ([src/popup/HeavyModelBanner.tsx](src/popup/HeavyModelBanner.tsx)). Heavy models are **warned, never blocked**.
- **Default unchanged**: packaged `ollamaModel` stays `llama3.2:3b` (no broken first run); the catalog nudges up.
- **Docs**: [docs/TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md) linked from the Options card, the popup error states, and the README. 568 unit tests pass.

### 20. Webmail/inbox false positive — exclude grid/feed item-selector checkboxes
- **2026-06-30**: Awto fired on the Gmail inbox and proposed mapping profile values (LinkedIn URL, phone, email) into email rows. **Root cause (reproduced)**: Gmail's per-email select-checkbox is a `[role="checkbox"]` whose accessible name is the email summary. The ARIA scanner captured every `[role="checkbox"]`, so dozens of email rows became "fields" whose labels carry email *content* keywords (`linkedin`→work, `phone`→contact, `github`→web); ≥3 distinct categories tripped the detector's `categories.size >= 3` gate, and the LLM then mapped values into them.
- **Fix** ([src/content/form-scanner.ts](src/content/form-scanner.ts)): the `[role="checkbox"]` ARIA query gains `skipIfInside: '[role="grid"], [role="row"], [role="table"], [role="treegrid"], [role="rowgroup"], [role="feed"]'`. Checkboxes inside a grid/table/feed are row/item selectors (Gmail, data tables, social feeds), not fillable form controls — excluding them at the **scanner** means neither the detector counts them nor the LLM ever sees them. Standalone consent checkboxes (not in a collection container) are unaffected. Repro: 13 scanned fields → 1 (search box only), detector 13 → 0. 547 unit tests pass.

### 21. Brand mark — "Pen & Pencil A" logo, on everything
- **2026-07-01**: Imported the finalized brand mark from a claude.ai/design handoff (project `444d6b2c…`, file "Awto Logo - Pen & Pencil A"). The mark is an abstract "A" built from a crossed pencil (+26°) and pen (−26°) sharing a pivot at (50, 25.75) in a 100×100 space, resting on a short "field line" crossbar — the two tools that fill a form. Tokens: accent `#4ade80`, knocked-out mark on tile `#08221a`, dark surface `#0a0a0c`.
- **Single source of truth**: [src/shared/logo.ts](src/shared/logo.ts) `buildAwtoLogoSvg({size, variant, surface, title})` returns the SVG string (framework-agnostic); [src/shared/AwtoLogo.tsx](src/shared/AwtoLogo.tsx) wraps it for React. Two variants: `tile` (green rounded-square, dark mark) and `mono` (green mark on transparent/dark, slit painted in the surface color). Geometry reproduced as `<rect rx>` + `<polygon>` inside `rotate()` groups (the barrel's bottom rounding is hidden under the nib/point triangles, so `rx` on all corners is exact).
- **Applied on everything**: manifest PNG icons 16/48/128 (regenerated from the tile SVG via macOS `sips`, which rasterizes SVG→PNG with no extra dep), popup Header avatar, content-script pill avatar, About-tab lockup, and an SVG favicon ([src/assets/logo.svg](src/assets/logo.svg)) linked from both popup/options HTML. Legible to 16px. 576 unit tests pass.

### 22. Settings (Options page) redesign + multi-provider cloud (design-first)
- **2026-07-01**: Imported the "Awto Settings" design from the same claude.ai/design project and reskinned the Options page. It is a **visual/layout redesign only** — the profile schema and `useOptionsState` already backed every field/control, so no data-model change. Layout moved from a horizontal tablist to a **fixed left sidebar** (`role="tablist"` vertical: Profile / LLM & Models / About) + scrolling main. New palette lives in [src/options/styles.css](src/options/styles.css) tokens (bg `#090c12`, panel `#0f141c`, input `#0a0e15`, text `#e8edf3`, borders `rgba(148,163,184,.1)`), fonts Hanken Grotesk + JetBrains Mono. **Accent stays the brand green `#4ade80`** (user chose brand consistency over the design's `#34d399`); the cloud/fallback accent is indigo `#818cf8`.
- **Profile**: sticky header with a completion **ring** (% of built-in fields filled) + section jump-chips; each section is an icon-headed card. The Identification section is amber/sensitive with ON-DEVICE badges, a shield note, and a Show/Hide reveal (inputs masked as `password` by default). Custom fields + Raw JSON moved into an "Advanced" section.
- **LLM**: local→cloud pipeline strip, local card (keeps `ModelCatalog` untouched), cloud card, and the ARIA-forms toggle. Threshold is a gradient slider (indigo cloud-side → green local-side).
- **Multi-provider cloud — now wired (was design-first in the first pass)**: the cloud Provider dropdown ([src/options/LLMTab.tsx](src/options/LLMTab.tsx) `PROVIDERS`) supports Anthropic / OpenAI / Google Gemini / OpenRouter / Custom, all functional. Key insight: OpenAI, Gemini, OpenRouter and Custom all speak the **OpenAI-compatible `/chat/completions`** API (Gemini via `…/v1beta/openai`), so one `fetch` in [cloud.ts](src/background/llm/cloud.ts) `callOpenAICompatible` (with `response_format: json_schema`) covers all four; Anthropic keeps its existing SDK path. `resolveCloud(opts)` dispatches by `cloudProvider`. Storage: Anthropic still uses `anthropicApiKey`/`anthropicModel` (back-compat); non-Anthropic persists per-provider under `cloudApiKeys`/`cloudModels` + `cloudBaseUrl` for Custom. `hybrid.ts` `canEscalateToCloud` checks the resolved provider's key. No new deps (native `fetch`).
- Preserved all test-critical hooks (nav `role="tab"` names, `Title`/`Custom Title` enum select + `__custom__` sentinel, `Fill custom-widget forms` toggle label, ModelCatalog `data-model`/`data-action`). Dropped `backdrop-filter: blur` on the sticky header (paint-artifact risk). 576 unit tests pass; build clean.
