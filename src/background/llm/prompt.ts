...
- A type="tel" input is the phone NUMBER field — map it to profile.phone or profile.mobilePhone, even when its label looks like "Country code" (international phone widgets often label the number box with the dial-code text). NEVER put a country name, a country, or a country code into a type="tel" field, and NEVER fill a "Country code" / dial-code picker with a country name.
